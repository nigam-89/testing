        ...
        <div className="flex flex-col-reverse justify-between md:flex-row">
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3  gap-x-5 md:gap-x-8">
            {data?.map((item, index) => {
              ...
              return (
                <Link to={`/in-media/${updatedTitle}/${item._id}`} key={index} className="flex">
                  <div className="shadow-2xl rounded-2xl mb-16 pb-2 relative group before:absolute before:h-[6px] before:bottom-0 before:bg-gradient-to-r from-[#1a3777] to-[#1a3777] before:left-0 before:w-full before:transition-all before:duration-500 hover:before:h-full before:-z-10 before:rounded-2xl flex flex-col justify-between">
                    <img
                      className="object-cover max-h-[15rem] h-[15rem] w-full rounded-lg"
                      src={
                        item.thumbnailId
                          ? item.thumbnailId
                    ...
                    />
                      ...
                      <p className="font-normal font-[Poppins] group-hover:text-white duration-150 text-sm tracking-wide text-gray-700 mb-4  dark:text-white dark:group-hover:text-black">
                          ...
                          {allMonths.length > 0 &&
                            allMonths.map((item, index) => {
                              return (
                                ...
                                <Link
                                  to={`/in-media-month/${item.count}-${yearData}`}
                                >
                                  <li
                                    key={index}
