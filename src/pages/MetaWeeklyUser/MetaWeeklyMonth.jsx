...
const MetaWeeklyMonth = () => {
    ...
    const getData = async (pageNo = 1, pageSize = 10) => {
        ...
        setLoading(true);
        try {
            const response = await apiConnector({ method: "GET", url: MetaWeeklyAPI.ViewMeta_API + `?pageNo=${pageNo}&pageSize=${pageSize}&dateDescSort=true&date=${month_year.split("-")[1]}-${month_year.split("-")[0]}-01`, })
    ...
    useEffect(() => {
        ...
        const fetchAllYears = async (pageNo = 1, pageSize = 10000) => {
            try {
                const res = await apiConnector({ method: "GET", url: MetaWeeklyAPI.ViewMeta_API + `?pageNo=${pageNo}&pageSize=${pageSize}&dateDescSort=true` })
                let arr = [];
                ...
                <div className='w-full min-h-screen relative py-5' >
                    <div className='flex justify-center items-centercmb-3 md:mb-5'>
                        <div className='h-[2.5rem]'>
                            <img src="https://res.cloudinary.com/djr2f6dlh/image/upload/v1699427105/key_perosn/metaweekly-logo_xhwp4m.webp" className='h-full w-full object-contain' alt="" />
                        </div>
                    </div>
                    ...
                    <div className='flex flex-col-reverse justify-between md:flex-row'>
                        {
                            loading ?
                                <>
                                    <div className='w-full flex items-center justify-center my-20'> <Loader width={"100"} color="black" height={"80"} /> </div>
                                </>
                                :
                                                ...
                                                <Tbody>
                                                    {
                                                        ...
                                                        data?.map((item, index) => {
                                                            return (
                                                                <Tr className={`${index % 2 === 0 ? 'text-black' : 'text-white bg-[#1a3777]'}`} key={index} >
                                                                    <Td>
                                                                        <p className='font-[Poppins] text-sm text-[700]  tracking-wide'>{(formula - index)}</p>
                                                                    </Td>
                                                                    <Td>
                                                                    ...
                                                                    <Td>
                                                                        <p className='font-[Poppins] text-sm text-[500] tracking-wide'>{item.volumeHeading}</p>
                                                                    </Td>
                                                                    <Td
                                                                    >
                                                        ...
                                                        <ul className='mt-0'>
                                                            {
                                                                ...
                                                                allMonths.map((item, index) => {
                                                                    return (
                                                                        ...
                                                                        <Link to={`/metaweekly-month/${item.count}-${yearData}`}>
                    ...
                    <div className='w-full my-3 px-4 md:px-32 h-full bg-blue-400 rounded-md bg-clip-padding backdrop-filter backdrop-blur-sm bg-opacity-10 '>
                        <div className='w-[100%] mx-auto flex justify-between items-center py-4 '>
                            <div className='flex justify-between items-center gap-6'>
                                <Button isDisabled={loading} colorScheme='facebook' size='sm' onClick={() => descreasePageNo(pageNo)}>
                                    <p className='font-[Poppins] font-[400] flex justify-center items-center'><AiOutlineLeft className='mr-2' />Prev </p>
                                </Button>
                                <Button isDisabled={loading} colorScheme='facebook' size='sm' onClick={() => increasePageNo(pageNo)}>
...
export default MetaWeeklyMonth;