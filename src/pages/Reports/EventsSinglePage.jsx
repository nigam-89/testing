...
const EventsSinglePage = () => {
    ...
    const getSingleData = async () => {
        setLoading(true);
        try {
            const response = await apiConnector({ method: "GET", url: EventWebinarAPI.ViewSingleEventWebinar_API + `?dbId=${id}` })
    ...
    const postComment = async (req, res) => {
        setLoading(true)
        try {
            await apiConnector({ method: "POST", url: EventWebinarAPI.PostingACommentEventWebinar_API, bodyData: formData, headers: { token: user.token ? user.token : admin.token } })
    ...
    const deleteComment = async (dbId, commentDBId) => {
        try {
            await apiConnector({ method: "DELETE", url: EventWebinarAPI.DeleteACommentEventWebinar_API + `?dbId=${dbId}&commentDBId=${commentDBId}`, headers: { token: admin.token } })
            getSingleData();
        } catch (error) {
            toast.error(error.message)
        }
        ...
        <div>
            ...
            <div className='3xl:w-[1500px] mx-auto'>
                <div className='h-[100%] w-[100%] overflow-x-hidden'>
                    <div className='w-[95%] mx-auto py-8 flex flex-col md:flex-row justify-between items-center gap-2'>
                        <img alt='' src={data?.imgUrlModelDBId?.urls[0]} className='w-[100%] md:w-[354px] h-[500px]' />
                        <div className='relative w-[100%] md:w-[90%] md:max-w-[85%] h-[200px] md:h-[500px]'>
                            {/* <img alt='' src={data?.imgUrlModelDBId?.urls[0]} className='w-[100%] h-[300px] ' /> */}
                            <iframe width="100%" height="100%" src={`https://www.youtube.com/embed/${data?.ytb?.ytbUrl}`} title="YouTube video player" frameborder="0" allow="" allowfullscreen></iframe>
                    ...
                    <div className='w-[95%] mx-auto flex flex-col justify-between items-start gap-4 mt-8 py-8'>
                        ...
                        <p className='font-semibold'>{moment(data.date).format('Do MMM YYYY, dddd')} / Event</p>
                    </div>

                    {
                        (user.hasSubscription || admin.token) ? (
                            <div className='w-[95%] mx-auto flex flex-col justify-between items-start gap-6 py-4'>
                                ...
                                <Link to={data?.pdfLink} target="_blank">
                                    <p className='border px-8 py-2 bg-[#243b77] text-white font-[Rubik] rounded-md'>Download</p>
                                </Link>
                            </div>
                        ) : (
                        ...
                        <ModalContent bg='rgb(255 145 3 / 1%)' p='6' rounded='xl'>
                            ...
                            <ModalBody mx='auto' display="flex" alignItems="center" justifyContent="center" overflow="hidden">
                                <div className='hidden md:block'>
                                    <iframe width="820" height="450" src={`https://www.youtube.com/embed/${data?.ytb?.ytbUrl}?autoplay=1&mute=1&enablejsapi=1`} title="YouTube video player" allowFullScreen style={{ borderRadius: "2rem" }}></iframe>
                                </div>
                                <div className='md:hidden'>
                                    <iframe width="300" height="400" src={`https://www.youtube.com/embed/${data?.ytb?.ytbUrl}?autoplay=1&mute=1&enablejsapi=1`} title="YouTube video player" allowFullScreen style={{ borderRadius: "2rem" }}></iframe>
                                </div>
                            </ModalBody>
                        </ModalContent>
                    </Modal>
