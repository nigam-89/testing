...
const OpinionBox = () => {
    ...
    const fetchAllOpinionBox = async (req, res, pageNo = 1, pageSize = 15) => {
      ...
      try {
        const response = await apiConnector({
          method: "GET",
          url:
            OpinionBoxAPI.ViewOpinionBox_API +
  ...
  // Handling Meta Tags
  const description =
    "METALOGIC Is specialized In corporate events management in which it will provide 'A to Z' services from conceptualizing the event to Inviting the right targeted audience along with speakers, sponsors, chief guests and other dignitaries.";
  ...
  return (
    <div>
      ...
      <div className="3xl:w-[1500px] mx-auto">
        <div className="px-3 md:px-10 py-4 dark:bg-black">
          <h2 className="font-[Poppins] text-left tracking-wider text-3xl font-[600] pb-8 pt-8">
            OPINION BOX
          </h2>
          <div className="flex flex-col-reverse justify-between md:flex-row">
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-x-5 md:gap-x-8">
              {data?.length > 0 ? (
                data.map((item, index) => {
                  updatedTitle = item?.heading.split(" ").join("-");

                  return (
                    <Link to={`/opinion-box/${updatedTitle}/${item._id}`} key={index} className="flex">
                      <div className="shadow-2xl rounded-2xl mb-16 pb-2 relative group before:absolute before:h-[6px] before:bottom-0 before:bg-gradient-to-r from-[#1a3777] to-[#1a3777] before:left-0 before:w-full before:transition-all before:duration-500 hover:before:h-full before:-z-10 before:rounded-2xl flex flex-col justify-between">
                       <div> <img
                          className="object-cover h-[15rem] max-h-[15rem] w-[320px] mx-auto  rounded-lg"
                          src={
                            item.thumbnailId
                              ? item.thumbnailId
                        ...
                        <div className="p-5 flex flex-col justify-between flex-grow">
                          ...
                          <h5 className="text-black group-hover:text-white duration-150 font-[600] font-[Nunito] text-lg tracking-wider mb-3 dark:text-white dark:group-hover:text-black">
                            {item.heading}
                          </h5>
                          <p className="font-normal font-[Poppins] group-hover:text-white duration-150 text-sm tracking-wide text-gray-800 mb-3 dark:text-white dark:group-hover:text-black">
                ...
                <h2 className="font-[Rubik] text-xl px-4 md:text-3xl flex justify-between items-start max-md:my-8">
                  No Data Found
                </h2>
              )}
            </div>
            <div className="min-w-[20%] flex flex-col py-4 md:py-2 px-8 md:mt-20 h-fit ">
                            ...
                            {allMonths.length > 0 &&
                              allMonths.map((item, index) => {
                                return (
                                  ...
                                  <Link
                                    to={`/opinion-box-month/${item.count}-${yearData}`}
                                  >
                                    <li
                                      key={index}
