    ...
    const getData = async (pageNo = 1, pageSize = 12) => {
      ...
      try {
        const response = await apiConnector({
          method: "GET",
          url:
            TopStoiresAPI.ViewTopStories_API +
      ...
      <div className="3xl:w-[1500px] mx-auto">
        <div className="px-3 md:px-10 py-4 dark:bg-black">
          ...
          <div className="flex flex-col-reverse justify-between md:flex-row">
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-x-5 md:gap-x-8">
              {data?.length > 0 ? (
                data.map((item, index) => (
                  <Link to={`/top-stories/${item._id}`} key={index} className="flex">
...
<div className="p-5 flex flex-col justify-between flex-grow">
                        ...
                        <p className="font-normal font-[Poppins] group-hover:text-white duration-150 text-sm tracking-wide text-gray-800 mb-3">
                          {moment(item.date).format("Do MMM YYYY, dddd")} / Top
                          Stories
                        </p>
