...
const TopStoriesSinglePage = () => {
    ...
    const getSingleData = async () => {
        ...
        setLoading(true);
        try {
            const response = await apiConnector({ method: "GET", url: TopStoiresAPI.SingleTopStories_API + `?dbId=${id}` })
    ...
    const postComment = async (req, res) => {
        setLoading(true)
        try {
            await apiConnector({ method: "POST", url: TopStoiresAPI.PostingACommentTopStories_API, bodyData: formData, headers: { token: user.token ? user.token : admin.token } })
    ...
    const deleteComment = async (dbId, commentDBId) => {
        try {
            await apiConnector({ method: "DELETE", url: TopStoiresAPI.DeleteACommentTopStories_API + `?dbId=${dbId}&commentDBId=${commentDBId}`, headers: { token: admin.token } })
            getSingleData();
        } catch (error) {
            toast.error(error.message)
        }
            ...
            <div className='3xl:w-[1500px] mx-auto'>
                <div className='h-[100%] w-[100%] overflow-x-hidden'>
                    ...
                    <div className='w-[95%] mx-auto flex flex-col justify-between items-start gap-4 mt-8 py-8'>
                        ...
                        <p className='font-semibold'>{moment(data.date).format('Do MMM YYYY, dddd')} / Top Stories</p>
                    </div>
...
export default TopStoriesSinglePage
