...
const SubscribedEmails = () => {
    ...
    const getData = async (pageNo = 1, pageSize = 15) => {
        ...
        setDataLoading(true);
        try {
            const response = await apiConnector({ method: "GET", url: SubscribedEmailsAPI.ViewSubsEmail_API + `?pageNo=${pageNo}&pageSize=${pageSize}&dateDescSort=true`, headers: { token: admin.token } })
    ...
    const deleteContact = async (id) => {
        try {
            ...
            const res = await apiConnector({ method: "DELETE", url: SubscribedEmailsAPI.DeleleSubsEmail_API + `/${id}`, headers: { token: admin.token } })
            if (res?.data?.success) {
