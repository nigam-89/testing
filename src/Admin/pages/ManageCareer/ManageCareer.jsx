...
const ManageCareer = () => {
    const [jobData, setJobData] = useState({ position: '', count: '', salary: '', active: true, link: '' })
    ...
    const createJob = async () => {
        ...
        try {
            ...
            setJobData({ position: '', count: '', salary: '', active: true, link: '' })
            setRequirementArray(['', ''])
    ...
    useEffect(() => {
        const fetchAllJobs = async () => {
            try {
                const response = await apiConnector({ method: "GET", url: CareerAPI.fetchAllListedJosbs })
                setAllJobs(response?.data.data)
                console.log("admin panel jobs", response?.data.data)
            } catch (error) {
                toast.error(error?.response?.data?.message)
            }
        }
            ...
            <div className='flex flex-col justify-between gap-8'>
                <section className="">
                    <div className="flex flex-col items-center justify-center md:px-6 py-8 mx-auto ">
                        <div className="w-[90%] md:w-[80%] bg-white rounded-lg shadow-2xl mx-auto overflow-x-hidden">
                            <div className="p-6 space-y-4 md:space-y-6 sm:p-8">
                                ...
                                <form className="space-y-4 md:space-y-6" onSubmit={handleSubmit}>
                                    <div>
                                        <label htmlFor="position" className="block mb-2 text-sm font-medium text-gray-900  text-start">Job Position</label>
                                        <input type="text" name="position" id="position" className="bg-gray-50 border border-gray-300 text-gray-900 sm:text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 " placeholder="Job Title"  autoComplete="off" onChange={(e) => ChangeHandler(e)} value={jobData.position} />
                                    </div>
                                    <div>
                                        <label htmlFor="count" className="block mb-2 text-sm font-medium text-gray-900  text-start">Openings</label>
                                        <input type="number" name="count" id="count" className="bg-gray-50 border border-gray-300 text-gray-900 sm:text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 " placeholder="Number of Opening"  autoComplete="off" onChange={(e) => ChangeHandler(e)} value={jobData.count} />
                                    </div>
                                    <div>
                                        <label htmlFor="salary" className="block mb-2 text-sm font-medium text-gray-900  text-start">Salary</label>
                                        <input type="text" name="salary" id="salary" placeholder="Enter The Offering Salary" className="bg-gray-50 border border-gray-300 text-gray-900 sm:text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 "  autoComplete="off" onChange={(e) => ChangeHandler(e)} value={jobData.salary} />
                                    </div>

                                    <div>
                                        <label htmlFor="link" className="block mb-2 text-sm font-medium text-gray-900  text-start">Link to Apply</label>
                                        <input type="text" name="link" id="link" placeholder="Enter Form Link" className="bg-gray-50 border border-gray-300 text-gray-900 sm:text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 "  autoComplete="off" onChange={(e) => ChangeHandler(e)} value={jobData.link} />
                                    </div>
                        ...
                        {
                            loading ? (<div><Loader /></div>) : allJobs.length === 0 ? (<p className='my-4 text-[3rem] text-white text-center font-[Rubik] mt-8'>No Current Openings</p>) : (
                                <div className='w-[100%] gap-4 py-8 mt-6 flex flex-col'>
                                    <h2 className='my-4 text-[2rem] md:text-[3rem] text-center font-[Rubik] text-white'>All Current Openings</h2>
                                    {
                                                ...
                                                <div key={index} className='w-[100%] bg-[#0c3a7c] mt-8' style={{ borderRadius: "80px 0px 130px 50px" }}>
                                                    <div className='flex flex-col items-start  bg-white md:flex-col md:border py-8' style={{ borderRadius: "100px 0px 150px 20px" }}>
                                                        ...
                                                        <div className='flex flex-col justify-between items-center gap-2 md:flex-row'>
                                                            <div>
                                                                {
                                                                    job.active ? (<button className='px-14 py-2 ml-8 bg-green-700 text-white rounded-lg text-sm font-[Rubik] mt-8 transition-all duration-300 hover:shadow-lg hover:scale-x-[1.05] ' onClick={() => updateHiringStatus(job._id)}>Activily Hiring</button>) : (<button className='px-14 py-2 ml-8 bg-red-700 text-white rounded-lg text-sm font-[Rubik] mt-8 transition-all duration-300 hover:shadow-lg hover:scale-x-[1.05] ' onClick={() => updateHiringStatus(job._id)}>Hiring Closed</button>)
                                                                }
                                                            ...
                                                            <Link target="_blank" to={job.link} className='px-14 py-2 ml-8 bg-[#0c3a7c] text-white rounded-lg text-sm font-[Rubik] mt-8 transition-all duration-300 hover:shadow-lg hover:scale-x-[1.05]'>Open Form</Link>
                                                            <button className='px-14 py-2 ml-8 bg-red-600 text-white rounded-lg text-sm font-[Rubik] mt-8 transition-all duration-300 hover:shadow-lg hover:scale-x-[1.05] ' onClick={() => deleteJob(job._id)}>Delete Job</button>
                                                        </div>
                                                    </div>
                                                </div>
                                            )
