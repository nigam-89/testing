...
const EditLatestVideos = ({ item }) => {
    ...
    const [formData, setFormData] = useState({ heading: '', shortDesc: '', briefDesc: '', ytbUrl: '' })
    ...
    const handleEdit = async () => {
        setLoading(true)
        try {
            const response = await apiConnector({ method: "GET", url: LatestVideosAPI.SingleLatestVideos_API + `?dbId=${item._id}`, headers: { token: admin.token } })

            setFormData({ heading: response.data.data.heading, shortDescription: response.data.data.shortDescription, briefDesc: response.data.data.briefDescription, ytbUrl: response.data.data.ytbUrl })
        } catch (error) {
        ...
        try {
            ...
            if (images) {
                response = await apiConnector({
                    method: "PUT",
                    url: LatestVideosAPI.UpdateLatestVideos_API + `/${item._id}`,
                    bodyData: {
                ...
                if (response?.data?.success) {
                    ...
                    setFormData({ heading: '', shortDesc: '', briefDesc: '', ytbUrl: '' });
                    setSelectedImages([]);
                ...
                <ModalContent>
                    ...
                    <ModalBody>
                        <div className='flex flex-col gap-3'>
                            ...
                            <div className='flex font-[Nunito] flex-col gap-4 p-4 font-[600] rounded-lg text-gray-900 bg-gray-200'>
                                <p className='font-[800] text-blue-600'>Youtube Link</p>
                                <input type={"text"} name="ytbUrl" value={formData.ytbUrl} onChange={changeHandler} className="py-2 pl-2 bg-transparent border border-blue-600 rounded-lg" />
                            </div>
                            <div className='flex font-[Nunito] flex-col gap-4 p-4 font-[600] rounded-lg text-gray-900 bg-gray-200'>
...
export default EditLatestVideos
