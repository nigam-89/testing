...
const View = () => {
  ...
  const getData = async (pageNo = 1, pageSize = 10) => {
    ...
    setLoading(true);
    try {
      const response = await apiConnector({ method: "GET", url: OpinionBoxAPI.ViewOpinionBox_API + `?pageNo=${pageNo}&pageSize=${pageSize}&dateDescSort=true`, headers: { token: admin.token } })
  ...
  const HandleDelete = async (id,role) => {
    ...
    try {
      const res = await apiConnector({ method: "DELETE", url: OpinionBoxAPI.DeleteOpinionBox_API + `/${id}`, headers: { token: admin.token } })
      if (res?.data?.success) {
  ...
  return (
    <div className='w-full min-h-screen relative' >
      <h2 className='text-2xl font-[Poppins] text-center text-white font-[500] pb-4 tracking-wide'>OPINION BOX</h2>
          ...
          <div className='grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 2xl:grid-cols-4 4xl:grid-cols-5 gap-x-5 md:gap-x-8 '>
            {
              ...
              data?.map((item, index) => (
                ...
                <div key={index} className="shadow-2xl  rounded-2xl mb-16 pb-2 relative 
                            ...
                            bg-white  
                            ">
                  <Link to={`/opinion-box/${item.heading}/${item._id}`}>
                  ...
                  <Link to={`/opinion-box/${item.heading}/${item._id}`}>
                    <p className="font-normal font-[Poppins]  duration-150 text-sm tracking-wide text-gray-800 mb-3">{moment(item.date).format('Do MMM YYYY, dddd')} / Opinion Box</p>
                    <p className="font-normal font-[Poppins]  duration-150 text-sm tracking-wide text-gray-700 mb-4 ">{item.shortDescription}</p>
                    </Link>
