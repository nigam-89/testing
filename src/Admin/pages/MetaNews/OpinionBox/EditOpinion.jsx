...
import { HandleImagesAPI, OpinionBoxAPI } from '../../../../services/apis';
...
const EditOpinion = ({ item }) => {
    ...
    const handleEdit = async () => {
        setLoading(true)
        try {
            const response = await apiConnector({ method: "GET", url: OpinionBoxAPI.SingleOpinionBox_API + `?dbId=${item._id}`, headers: { token: admin.token } })
        ...
        try {
            ...
            if (images) {
                response = await apiConnector({
                    method: "PUT",
                    url: OpinionBoxAPI.UpdateOpinionBox_API + `/${item._id}`,
                    bodyData: {
