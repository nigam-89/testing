...
import { HandleImagesAPI, InMediaAPI } from '../../../services/apis';
import { apiConnector } from '../../../services/apiConnector';


const EditInMedia = ({ item }) => {
    ...
    const handleEdit = async () => {
        setLoading(true)
        try {
            const response = await apiConnector({ method: "GET", url: InMediaAPI.ViewSingleInMedia_API + `?dbId=${item._id}`, headers: { token: admin.token } })

            setFormData({ heading: response.data.data.heading, description: response.data.data.description, inMediaUrl: response?.data?.data?.inMediaUrl })
        } catch (error) {
        ...
        try {
            ...
            if (images) {
                response = await apiConnector({
                    method: "PUT",
                    url: InMediaAPI.UpdateInMedia_API + `/${item._id}`,
                    bodyData: {
                ...
                if (response?.data?.success) {
                    ...
                    setFormData({ heading: '', description: '', inMediaUrl: '' });
                    setSelectedImages([]);
                    ...
                    <ModalBody>
                        <div className='flex flex-col gap-3'>
                            ...
                            <div className='flex font-[Nunito] flex-col gap-4 p-4 font-[600] rounded-lg text-gray-900 bg-gray-200'>
                                ...
                                <input type={"text"} name="description" value={formData.description} onChange={changeHandler} className="py-2 pl-2 bg-transparent border border-blue-600 rounded-lg" />
                            </div>
                            <div className='flex font-[Nunito] flex-col gap-4 p-4 font-[600] rounded-lg text-gray-900 bg-gray-200'>
                                <p className='font-[800] text-blue-600'> InMedia URL</p>
                                <input type={"text"} name="inMediaUrl" value={formData.inMediaUrl} onChange={changeHandler} className="py-2 pl-2 bg-transparent border border-blue-600 rounded-lg" />
                            </div>
                            {
